...
class App extends Component {
  ...
  getAdminsCharities = () => {
    api.charities
      .getAdminsCharities(this.state.user.id)
      .then((data) => this.setState({ adminsCharities: data }));
  };

  getUserRequests = () => {
    api.requests
      .getUserRequests(this.state.user.id)
      .then((data) => this.setState({ userRequests: data }));
  };

  getCharityRequests = (charityId) => {
    api.requests
      .getCharityRequests(charityId, this.state.user.id)
  ...
  addRequest = (requestInfo, charityId, userId) => {
    api.requests.addRequest(requestInfo, charityId, userId).then((data) =>
      this.setState((prevState) => ({
  ...
  editUser = (userInfo) => {
    api.auth.editUser(userInfo, this.state.user.id).then((data) => {
  ...
  editCharity = (charityInfo, charityId) => {
    api.charities
      .editCharity(charityInfo, charityId, this.state.user)
      .then((data) => {
        let updatedCharity = this.state.allCharities.findIndex(
  ...
  editRequest = (requestInfo, requestId) => {
    api.requests.editRequest(requestInfo, requestId).then((data) => {
      let updatedRequest = this.state.charityRequests.findIndex(
  ...
  editRequestStatus = (requestId, status) => {
    api.requests.editRequestStatus(requestId, status).then((data) => {
      let updatedRequest = this.state.charityRequests.findIndex(
        (request) => request.id === requestId
      );
      let copyOfRequests = Object.assign([], this.state.charityRequests);
  ...
  editRequestStatusDonor = (requestId) => {
    api.requests.editRequestStatusDonor(requestId).then((data) => {
      let updatedUserRequest = this.state.userRequests.filter(
        (r) => r.id !== requestId
      );
      let updatedCharityRequest = this.state.charityRequests.findIndex(
        (r) => r.id === requestId
      );
  ...
  editRequestStatusAndId = (request, status) => {
    api.requests
      .editRequestStatusAndId(request, status, this.state.user.id)
      .then((data) => {
        let updatedRequest = this.state.charityRequests.findIndex(
          (r) => r.id === request.id
        );
  ...
  deleteCharitySet = (id) => {
    ...
    this.setState((prevState) => ({
      allCharities: prevState.allCharities.filter(
        (charity) => charity.id !== id
      ),
      adminsCharities: prevState.adminsCharities.filter(
        (charity) => charity.id !== id
      ),
    }));
  };
  ...
  charityRequestDelete = (requestId) => {
    this.setState((prevState) => ({
      charityRequests: prevState.charityRequests.filter(
        (request) => request.id !== requestId
      ),
    }));
  };
  ...
  listFilteredCharities = () => {
    const filterCharByCity = this.state.allCharities.filter((charity) =>
      charity.city.includes(this.state.city)
    );
